...
class PandasetGtDataset():
    def __init__(self, dataset_cfg=None, class_names=None, training=True, root_path=None, logger=None):
        ...
        if 'FOLD_IDX' in self.dataset_cfg:
            # db_infos_path = self.root_path / 'pandaset_dbinfos_train.pkl'
            db_infos_path_list = [self.root_path / 'pandaset_dbinfos_train.pkl']
            # db_infos_path_list = [self.root_path / 'pandaset_dbinfos_trainval.pkl']
            # db_infos_path_list = [self.root_path / 'pandaset_dbinfos_train.pkl', self.root_path / 'pandaset_dbinfos_val.pkl']
            infos_list = []
            for db_infos_path in db_infos_path_list:
                with open(db_infos_path, 'rb') as f:
                    infos = pickle.load(f)
                    infos_list.append(infos)
            splits = KFold(n_splits=2, shuffle=True, random_state=42)  # random_state=42
            ...
            if self.training:
                self.pandaset_infos = [used_infos[idx] for idx in train_idx]
            else:
                self.pandaset_infos = [used_infos[idx] for idx in val_idx]
        else:
            if self.training:
                db_infos_path = self.root_path / 'pandaset_dbinfos_train.pkl'
            else:
                db_infos_path = self.root_path / 'pandaset_dbinfos_val.pkl'

            with open(db_infos_path, 'rb') as f:
                infos = pickle.load(f)
            self.pandaset_infos = infos['Car']

        self.total_epochs = 0
        self._merge_all_iters_to_one_epoch = False

        # dense gt infos
        self.dense_gt_infos = [x for x in self.pandaset_infos if x['num_points_in_gt'] > 1000]
        ...
        logger.info(
            f"### Aug params: flip={self.enable_flip}, scale={self.scale_range}, rot={self.angle_rot_max},shift={self.pos_shift_max}, enable_similar_type={self.enable_similar_type}")
    ...
    def occlude_aug(self, sample_car_info, points, max_num=9999, min_num=1, max_try_time=5):
        self.logger.info(f"This is the sample_car_info: {sample_car_info} ")
        frame_id = sample_car_info['gt_idx']
    ...
    def __getitem__(self, index):
        ...
        info = copy.deepcopy(self.pandaset_infos[index])
    ...
    def __len__(self):
        if self._merge_all_iters_to_one_epoch:
            return len(self.pandaset_infos) * self.total_epochs
...
def build_dataloader(dataset_cfg, class_names, batch_size, dist, root_path=None, workers=4,
    ...
    dataset_map = {
        ...
        "PandasetGtDataset": PandasetGtDataset
    }
    dataset_class = dataset_map[dataset_cfg.get('DATASET', 'PandasetGtDataset')]
