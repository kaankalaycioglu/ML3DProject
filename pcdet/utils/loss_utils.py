...
class compute_vertex(Function):
        ...
        for iter in range(N):
            ...
            for i in range(4):
                ...
                if (
                    ...
                    intersections[
                        iter, num_of_intersections[iter] * 2
                    ] = np_corners_gboxes[iter, i * 2]
                    intersections[
                        iter, num_of_intersections[iter] * 2 + 1
                    ] = np_corners_gboxes[iter, i * 2 + 1]
            ...
            for i in range(4):
                ...
                if abab >= abap and abap >= 0 and adad >= adap and adap >= 0:
                    intersections[
                        iter, num_of_intersections[iter] * 2
                    ] = np_corners_qboxes[iter, i * 2]
                    intersections[
                        iter, num_of_intersections[iter] * 2 + 1
                    ] = np_corners_qboxes[iter, i * 2 + 1]
                    num_of_intersections[iter] += 1
                        ...
                        if abc != abd:
                            ...
                            if num_of_intersections[iter] > 7:
                                ...
                                print(
                                    ...
                                    % (
                                        np_corners_gboxes[iter, 0],
                                        np_corners_gboxes[iter, 1],
                                        np_corners_gboxes[iter, 2],
                                        np_corners_gboxes[iter, 3],
                                        np_corners_gboxes[iter, 4],
                                        np_corners_gboxes[iter, 5],
                                        np_corners_gboxes[iter, 6],
                                        np_corners_gboxes[iter, 7],
                                    )
                                )
                                print(
                                    "(%.4f %.4f) (%.4f %.4f) (%.4f %.4f) (%.4f %.4f)"
                                    % (
                                        np_corners_qboxes[iter, 0],
                                        np_corners_qboxes[iter, 1],
                                        np_corners_qboxes[iter, 2],
                                        np_corners_qboxes[iter, 3],
                                        np_corners_qboxes[iter, 4],
                                        np_corners_qboxes[iter, 5],
                                        np_corners_qboxes[iter, 6],
                                        np_corners_qboxes[iter, 7],
                                    )
                                )
                                continue
                            intersections[iter, num_of_intersections[iter] * 2] = (
                                Dx / DH
                            )
                            intersections[iter, num_of_intersections[iter] * 2 + 1] = (
                                Dy / DH
                            )
                            num_of_intersections[iter] += 1
    ...
    def backward(ctx, *grad_outputs):
                    ...
                    if flags_inters[iter, i, j] > 0:
                        ...
                        if acd != bcd:
                            ...
                            if abc != abd:
                                ...
                                # DH = (B[1] - A[1]) * (D[0] - C[0]) - (B[0] - A[0]) * (D[1] - C[1])
                                # Dx = (A[0] * B[1] - B[0] * A[1]) * (D[0] - C[0]) - (B[0] - A[0]) * (C[0] * D[1] - D[0] * C[1])
                                det_DxA0 = B[1] * (D[0] - C[0]) + (
                                    C[0] * D[1] - D[0] * C[1]
                                )
                                det_DxA1 = -B[0] * (D[0] - C[0])
                                det_DxB0 = -A[1] * (D[0] - C[0]) - (
                                    C[0] * D[1] - D[0] * C[1]
                                )
                                det_DxB1 = A[0] * (D[0] - C[0])
                                det_DxC0 = (
                                    -(A[0] * B[1] - B[0] * A[1]) - (B[0] - A[0]) * D[1]
                                )
                                det_DxC1 = (B[0] - A[0]) * D[0]
                                det_DxD0 = (A[0] * B[1] - B[0] * A[1]) + (
                                    B[0] - A[0]
                                ) * C[1]
                                det_DxD1 = -(B[0] - A[0]) * C[0]
                                # Dy = (A[0] * B[1] - B[0] * A[1]) * (D[1] - C[1]) - (B[1] - A[1]) * (C[0] * D[1] - D[0] * C[1])
                                det_DyA0 = B[1] * (D[1] - C[1])
                                det_DyA1 = -B[0] * (D[1] - C[1]) + (
                                    C[0] * D[1] - D[0] * C[1]
                                )
                                det_DyB0 = -A[1] * (D[1] - C[1])
                                det_DyB1 = A[0] * (D[1] - C[1]) - (
                                    C[0] * D[1] - D[0] * C[1]
                                )

                                det_DyC0 = -(B[1] - A[1]) * D[1]
                                det_DyC1 = (
                                    -(A[0] * B[1] - B[0] * A[1]) + (B[1] - A[1]) * D[0]
                                )
                                det_DyD0 = (B[1] - A[1]) * C[1]
                                det_DyD1 = (A[0] * B[1] - B[0] * A[1]) - (
                                    B[1] - A[1]
                                ) * C[0]
                                # DH = (B[1] - A[1]) * (D[0] - C[0]) - (B[0] - A[0]) * (D[1] - C[1])
                                det_DHA0 = D[1] - C[1]
                                ...
                                Jacbian_gboxes[iter, i * 2, n_of_inter[iter] * 2] += (
                                    det_DxA0 * DH - Dx * det_DHA0
                                ) / DHDH
                                Jacbian_gboxes[
                                    iter, i * 2, n_of_inter[iter] * 2 + 1
                                ] += (det_DyA0 * DH - Dy * det_DHA0) / DHDH

                                Jacbian_gboxes[
                                    iter, i * 2 + 1, n_of_inter[iter] * 2
                                ] += (det_DxA1 * DH - Dx * det_DHA1) / DHDH
                                Jacbian_gboxes[
                                    iter, i * 2 + 1, n_of_inter[iter] * 2 + 1
                                ] += (det_DyA1 * DH - Dy * det_DHA1) / DHDH

                                Jacbian_gboxes[
                                    iter, 2 * ((i + 1) % 4), n_of_inter[iter] * 2
                                ] += (det_DxB0 * DH - Dx * det_DHB0) / DHDH
                                Jacbian_gboxes[
                                    iter, 2 * ((i + 1) % 4), n_of_inter[iter] * 2 + 1
                                ] += (det_DyB0 * DH - Dy * det_DHB0) / DHDH

                                Jacbian_gboxes[
                                    iter, 2 * ((i + 1) % 4) + 1, n_of_inter[iter] * 2
                                ...
                                Jacbian_qboxes[iter, j * 2, n_of_inter[iter] * 2] += (
                                    det_DxC0 * DH - Dx * det_DHC0
                                ) / DHDH
                                Jacbian_qboxes[
                                    iter, j * 2, n_of_inter[iter] * 2 + 1
                                ] += (det_DyC0 * DH - Dy * det_DHC0) / DHDH

                                Jacbian_qboxes[
                                    iter, j * 2 + 1, n_of_inter[iter] * 2
                                ] += (det_DxC1 * DH - Dx * det_DHC1) / DHDH
                                Jacbian_qboxes[
                                    iter, j * 2 + 1, n_of_inter[iter] * 2 + 1
                                ] += (det_DyC1 * DH - Dy * det_DHC1) / DHDH

                                Jacbian_qboxes[
                                    iter, 2 * ((j + 1) % 4), n_of_inter[iter] * 2
                                ] += (det_DxD0 * DH - Dx * det_DHD0) / DHDH
                                Jacbian_qboxes[
                                    iter, 2 * ((j + 1) % 4), n_of_inter[iter] * 2 + 1
                                ] += (det_DyD0 * DH - Dy * det_DHD0) / DHDH

                                Jacbian_qboxes[
                                    iter, 2 * ((j + 1) % 4) + 1, n_of_inter[iter] * 2
        ...
        tensor_Jacbian_gboxes = torch.from_numpy(Jacbian_gboxes).to(
            torch.device(corners_qboxes.device)
        )
        tensor_Jacbian_qboxes = torch.from_numpy(Jacbian_qboxes).to(
            torch.device(corners_qboxes.device)
        )
        grad_output_cuda = grad_output.to(torch.device(corners_qboxes.device))
        # print("grad_output_cuda =", grad_output_cuda.shape)
        tensor_grad_corners_gboxes = tensor_Jacbian_gboxes.matmul(
            grad_output_cuda.unsqueeze(2)
        ).squeeze(2)
        tensor_grad_corners_qboxes = tensor_Jacbian_qboxes.matmul(
            grad_output_cuda.unsqueeze(2)
        ).squeeze(2)
...
class sort_vertex(Function):
    ...
    def forward(ctx, int_pts, num_of_inter):
        ...
        for iter in range(N):
            if np_num_of_inter[iter] > 0:
                ...
                for i in range(np_num_of_inter[iter]):
                    sorted_int_pts[iter, 2 * i] = np_int_pts[
                        iter, 2 * np_sorted_indexs[iter, i]
                    ]
                    sorted_int_pts[iter, 2 * i + 1] = np_int_pts[
                        iter, 2 * np_sorted_indexs[iter, i] + 1
                    ]
    ...
    def backward(ctx, grad_output):
        ...
        tensor_Jacbian_int_pts = torch.from_numpy(Jacbian_int_pts).to(
            torch.device(int_pts.device)
        )
        grad_output_cuda = grad_output.to(torch.device(int_pts.device))
        tensor_grad_int_pts = tensor_Jacbian_int_pts.matmul(
            grad_output_cuda.unsqueeze(2)
        ).squeeze(2)
                    ...
                    if i == 0:
                        for j in range(np_num_of_inter[iter] - 2):
                            ...
                            if (
                                ...
                                grad_int_pts[iter, 0] += (
                                    (p2[1] - p3[1]) * grad_output0[iter] * 0.5
                                )
                                grad_int_pts[iter, 1] += (
                                    -(p2[0] - p3[0]) * grad_output0[iter] * 0.5
                                )
                            else:
                                grad_int_pts[iter, 0] += (
                                    -(p2[1] - p3[1]) * grad_output0[iter] * 0.5
                        ...
                        if (
                            ...
                            grad_int_pts[iter, 2] = (
                                -(p1[1] - p3[1]) * grad_output0[iter] * 0.5
                            )
                            grad_int_pts[iter, 3] = (
                                (p1[0] - p3[0]) * grad_output0[iter] * 0.5
                            )
                        else:
                            grad_int_pts[iter, 2] = (
                                (p1[1] - p3[1]) * grad_output0[iter] * 0.5
                        ...
                        p1 = np_int_pts[
                            iter,
                            2
                            * (np_num_of_inter[iter] - 2) : 2
                            * (np_num_of_inter[iter] - 1),
                        ]
                        p2 = np_int_pts[
                            iter,
                            2
                            * (np_num_of_inter[iter] - 1) : 2
                            * (np_num_of_inter[iter]),
                        ]
                        ...
                        if (
                            ...
                            grad_int_pts[iter, 2 * (np_num_of_inter[iter] - 1)] = (
                                -(p1[1] - p3[1]) * grad_output0[iter] * 0.5
                            )
                            grad_int_pts[iter, 2 * np_num_of_inter[iter] - 1] = (
                                (p1[0] - p3[0]) * grad_output0[iter] * 0.5
                        ...
                        if (
                            ...
                            grad_int_pts[iter, i * 2] += (
                                (-(p2[1] - p3[1]) + (p1[1] - p3[1]))
                                * grad_output0[iter]
                                * 0.5
                            )
                            grad_int_pts[iter, i * 2 + 1] += (
                                (-(p1[0] - p3[0]) + (p2[0] - p3[0]))
                                * grad_output0[iter]
                                * 0.5
                            )
...
class rinter_area_compute(nn.Module):
    ...
    def forward(ctx, corners_gboxes, corners_qboxes):
        intersections, num_of_intersections = compute_vertex(
            corners_gboxes, corners_qboxes
        )
...
class find_convex_hull(Function):
    ...
    def forward(ctx, corners):
        ...
        tensor_hull_points_2d = torch.from_numpy(hull_points_2d).to(
            torch.device(corners.device)
        )
        ctx.index = index
...
class mbr_convex_hull(nn.Module):
    ...
    def forward(ctx, hull_points_2d):
        ...
        a = torch.stack(
            (torch.cos(edge_angles), torch.cos(edge_angles - 3.1415926 / 2.0)), 1
        )
        a = torch.unsqueeze(a, 1)
        b = torch.stack(
            (torch.cos(edge_angles + 3.1415926 / 2.0), torch.cos(edge_angles)), 1
        )
...
class mbr_area_compute(nn.Module):
    ...
    def forward(ctx, corners):
        ...
        mbr_rect_area = []
        for i in range(N):
            mbr_rect_area.append(
                torch.zeros((1,), dtype=torch.float32, device=corners.device)
            )
...
class mbr_diag_convex_hull(nn.Module):
    ...
    def forward(ctx, hull_points_2d):
        ...
        diag = torch.sqrt(
            (corner_max[0] - corner_min[0]) ** 2 + (corner_max[1] - corner_min[1]) ** 2
        )
        return diag
...
class align_inter_aligned(nn.Module):
    ...
    def forward(ctx, gboxes, qboxes):
        ...
        for i in range(N):
            iw = (
                min(rotated_corners1[i, 0, 2], rotated_corners2[i, 0, 2])
                - max(rotated_corners1[i, 0, 1], rotated_corners2[i, 0, 1])
                + eps
            )
            if iw > 0:
                ih = (
                    min(rotated_corners1[i, 1, 1], rotated_corners2[i, 1, 1])
                    - max(rotated_corners1[i, 1, 0], rotated_corners2[i, 1, 0])
                    + eps
                )
                if ih > 0:
                    inter_area_xoz[i] = iw * ih
            iwmbr = (
                max(rotated_corners1[i, 0, 3], rotated_corners2[i, 0, 3])
                - min(rotated_corners1[i, 0, 0], rotated_corners2[i, 0, 0])
                + eps
            )
            ihmbr = (
                max(rotated_corners1[i, 1, 1], rotated_corners2[i, 1, 1])
                - min(rotated_corners1[i, 1, 0], rotated_corners2[i, 1, 0])
                + eps
            )
...
class odiou_3D(nn.Module):
    ...
    def forward(ctx, gboxes, qboxes, weights, batch_size):
        ...
        corners_gboxes_1 = torch.stack(
            (corners_gboxes[:, [0, 2, 4, 6]], corners_gboxes[:, [1, 3, 5, 7]]), 2
        )
        corners_qboxes_1 = torch.stack(
            (corners_qboxes[:, [0, 2, 4, 6]], corners_qboxes[:, [1, 3, 5, 7]]), 2
        )
        ...
        inter_h = torch.min(
            gboxes[:, 2] + 0.5 * gboxes[:, 5], qboxes[:, 2] + 0.5 * qboxes[:, 5]
        ) - torch.max(
            gboxes[:, 2] - 0.5 * gboxes[:, 5], qboxes[:, 2] - 0.5 * qboxes[:, 5]
        )
        oniou_h = torch.max(
        ...
        dp = torch.div(
            center_dist_square_cuda[index_loc[:, 0]],
            mbr_diag_3d_square_cuda[index_loc[:, 0]],
        )
        try:
            odious[index_loc[:, 0]] = (
                1 - ious[index_loc[:, 0]] + dp + angle_factor[index_loc[:, 0]]
            )
        except:
...
if __name__ == "__main__":
    ...
    x = torch.tensor(
        [20.8845, -16.0514, -0.5310, 1.8061, 4.6556, 1.8546, 0.2290],
        dtype=torch.float32,
    ).view(1, 7)
    y = torch.tensor(
        [20.8869, -15.9686, -0.5253, 1.7909, 4.6727, 1.7605, 0.2375],
        dtype=torch.float32,
    ).view(1, 7)
